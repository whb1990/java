...
public class FindUnsortedSubarray {
     ...
     * <p>
     * 从头遍历 numsnums 数组，如果遇到的数字大小一直是升序的，就不断把对应的下标压入栈中，这么做的目的是因为这些元素在目前都是处于正确的位置上。
     * 一旦遇到前面的数比后面的数大，也就是 nums[j] 比栈顶元素小，就可以知道 nums[j] 一定不在正确的位置上。
     * <p>
     * 为了找到 nums[j]的正确位置，需要不断将栈顶元素弹出，直到栈顶元素比 nums[j]n 小，我们假设栈顶元素对应的下标为 k ，那么我们知道 nums[j] 的正确位置下标应该是 k + 1 。
     * <p>
